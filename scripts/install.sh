#!/bin/sh
# install.sh - instala/remove o dsh-explorer-plugin em um profile do DSH (Linux).
#
# Uso:
#   scripts/install.sh                instala o plugin (profile padrao: web)
#   scripts/install.sh --check        valida pre-requisitos sem instalar
#   scripts/install.sh --remove       remove o plugin do profile
#   scripts/install.sh --help         mostra esta ajuda
#
...
if [ -z "$NODE_BIN" ]; then
  err "Node.js nao encontrado no PATH. Instale Node.js >= 20 e tente novamente."
  exit 1
fi
...
if [ -z "$DSH_BIN" ]; then
  err "CLI 'dsh' nao encontrado no PATH. Instale com: npm install -g @deepseek-ai/dsh"
  exit 1
fi
ok "dsh ($DSH_BIN)"
...
if [ -z "$PNPM_BIN" ]; then
  ...
  if [ -f "$VENDORED_PNPM" ]; then
    ...
    err "Instale o pnpm (corepack enable / npm install -g pnpm) e tente novamente."
    exit 1
  fi
fi
...
Proximo passo - reinicie o servidor web para o novo bundle entrar no boot:
  * Se este ambiente tiver scripts/restart-web.sh:  scripts/restart-web.sh
  * Senao: pare o 'dsh web' (Ctrl+C) e suba de novo com 'dsh web'
  * Depois recarregue a GUI: http://127.0.0.1:3080

Para remover depois:  scripts/install.sh --remove
EOF
exit 0
