...
const puppeteer = require('/path/to/dsh-explorer-plugin/.pnpm-home/node_modules/puppeteer-core');
...
(async () => {
  const browser = await puppeteer.launch({
    ...
    headless: true,
    args: ['--no-sandbox', '-profile', '/path/to/dsh-explorer-plugin/.ff-profile'],
  });
  ...
  await page.screenshot({ path: '/path/to/dsh-explorer-plugin/.syntax-test.png' });
  await browser.close();
  process.exit(0);
