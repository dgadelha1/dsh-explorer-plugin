...
`;
		...
		// ───────────────────────── textmate ─────────────────────────
		// Load a UMD script so its factory lands on a known global even when
		// Monaco's AMD loader is present: its global `define` (with .amd) would
		// otherwise swallow the module via define([], factory) and never populate
		// window.onig / window.vscodetextmate. Shadowing module/exports routes the
		// UMD through its CommonJS branch; the value is read back from
		// module.exports and the globals are restored immediately.
		function loadScriptUmd(src, globalName) {
		...
		function ensureTextmate() {
			...
			textmateReady = (async function () {
				// Sequential on purpose: both UMDs share the shadowed module/exports
				// globals and each load restores them, so parallel loading would let
				// the second script execute after the first restored the globals.
				await loadScriptUmd("/explorer-assets/onig/main.js", "onig");
				await loadScriptUmd("/explorer-assets/textmate/main.js", "vscodetextmate");
		...
		// Monaco's IState contract (required by the editor since ~0.36): the
		// tokenization state must implement clone()/equals() or tokenizing a
		// model with a custom provider crashes with "s.clone is not a function".
		function TextMateState(stack) { this.stack = stack; }
		...
		TextMateState.prototype.equals = function (other) {
			...
			var a = this.stack, b = other.stack;
			if (a === b) return true;
			if (!a || !b) return false;
		...
		function installProvider(langId) {
			...
			ensureManifest().then(function () {
				...
				ensureGrammar(scope).then(function (g) {
					if (!g) return; // keep Monaco's built-in tokenizer instead of an empty provider
					monaco.languages.setTokensProvider(langId, {
						getInitialState: function () { return new TextMateState(null); },
						tokenize: function (line, state) {
							var g = loadedGrammars[scope];
							if (!g) return { tokens: [], endState: state };
							var stack = (state && state.stack) || null;
							var res = g.tokenizeLine(line, stack);
							var tokens = [];
							for (var i = 0; i < res.tokens.length; i++) {
								// Monaco's TokenTheme.match resolves ONE scope per token:
								// the space-joined VS Code scope stack yields the default
								// color in this Monaco build, so send the most specific
								// scope (last in the TextMate stack) — prefix rules such
								// as "comment" / "string" / "keyword" still match it.
								var scopes = res.tokens[i].scopes;
								tokens.push({ startIndex: res.tokens[i].startIndex, scopes: scopes[scopes.length - 1] });
							}
							return { tokens: tokens, endState: new TextMateState(res.ruleStack) };
						},
					});
